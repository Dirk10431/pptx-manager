...
//                  mit ExportHiddenSlides=true; Fallback PowerPoint via
//                  AppleScript (save as PDF)
//               2) JXA + PDFKit + NSBitmapImageRep: PDF -> pro Seite ein
//                  PNG, direkt in Zielgroesse (THUMB_WIDTH x THUMB_HEIGHT,
//                  ohne Retina-2x-Skalierung). Keine sips-Nachbearbeitung
//                  noetig.
...
function exportPptxAsPdfMacLibreOffice(filePath, pdfPath, timeoutMs) {
    return new Promise((resolve, reject) => {
        ...
        // ExportHiddenSlides=true: ausgeblendete Folien werden mit ins PDF
        // aufgenommen. Wichtig, weil der Scanner sie ebenfalls indiziert —
        // ohne diesen Filter waeren ihre Indizes "out-of-range" gegenueber
        // dem PDF, und die Thumbnail-Generierung wuerde sie ueberspringen.
        const pdfFilter = 'pdf:impress_pdf_Export:{"ExportHiddenSlides":{"type":"boolean","value":"true"}}';
...
//   - aus PDFPage.dataRepresentation entsteht ein NSImage
//   - das wird direkt in eine NSBitmapImageRep mit fixer Pixelgroesse
//     (THUMB_WIDTH x THUMB_HEIGHT) gerendert. Wichtig: NSImage.lockFocus
//     wuerde auf Retina-Displays mit 2x-Backing rendern (960x540); wir
//     bauen daher die Bitmap-Rep manuell, das ergibt echte 480x270-Pixel
//     in einem Schritt — keine Nachskalierung per sips noetig.
...
function run(argv) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        // Bitmap-Rep mit FIXEN Pixelmassen anlegen (kein Retina-Faktor).
        // Signatur: planes, w, h, bps, spp, hasAlpha, planar, colorSpaceName,
        //           bytesPerRow (0 = auto), bitsPerPixel (32 = RGBA 8bit)
        const rep = $.NSBitmapImageRep.alloc.initWithBitmapDataPlanesPixelsWidePixelsHighBitsPerSampleSamplesPerPixelHasAlphaIsPlanarColorSpaceNameBytesPerRowBitsPerPixel(
            $(), width, height, 8, 4, true, false, "NSCalibratedRGBColorSpace", 0, 32);
...
async function generateThumbnailsBatchMac(filePath, jobs, onProgress = null, options = {}) {
    ...
    try {
        ...
        // 2) PDF -> einzelne PNGs in Zielgroesse (480x270) — kein sips mehr noetig.
        const pageCount = await renderPdfPagesToPngsMac(pdfPath, pngDir, timeoutMs);

        // 3) Pro gewuenschtem Job: passende Seite an den finalen Pfad kopieren.
        for (const job of jobs) {
            ...
            try {
                fs.copyFileSync(srcPng, job.outPath);
