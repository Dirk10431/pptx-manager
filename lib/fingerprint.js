...
// Wird erhoeht, wenn sich die Hash-Berechnung aendert. Bei Versions-Mismatch
// zwischen Code und DB muss neu gescannt werden, sonst stimmen Duplikat-
// Gruppen nicht mehr.
//   1 = Erstversion
//   2 = Footer/Datum/Foliennummer aus Text- und Struktur-Hash ausgeblendet
...
/**
 * Prueft, ob eine Shape (p:sp) ein Footer-, Datums- oder Foliennummer-Platzhalter ist.
 * In PPTX steht das in p:nvSpPr > p:nvPr > p:ph > @_type:
 *   - 'ftr'    = Footer (Fusszeile)
 *   - 'dt'     = Date (Datumsfeld)
 *   - 'sldNum' = Slide Number (Foliennummer)
 * Diese Shapes werden aus Text- und Struktur-Hash ausgeblendet, damit sich
 * inhaltsgleiche Folien aus verschiedenen Seminaren als Duplikate erkennen lassen.
 */
const FOOTER_PLACEHOLDER_TYPES = new Set(['ftr', 'dt', 'sldNum']);

function isFooterShape(shape) {
    const phType = shape?.['p:nvSpPr']?.['p:nvPr']?.['p:ph']?.['@_type'];
    return FOOTER_PLACEHOLDER_TYPES.has(phType);
}

/**
 * Rekursiv alle Text-Knoten aus einem beliebigen XML-Unterknoten sammeln.
 * In PPTX sind Texte in <a:t>-Elementen (Attribute sind '@_' prefixed, Text in '#text').
 */
...
/**
 * Text einer Folie sammeln OHNE Footer/Datum/Foliennummer-Platzhalter.
 * Geht durch spTree, ueberspringt Footer-Shapes und sammelt aus dem Rest.
 */
function collectSlideText(slideObj) {
    const out = [];
    const spTree = slideObj?.['p:sld']?.['p:cSld']?.['p:spTree'];
    if (!spTree) return out;

    // Normale Shapes (p:sp) — mit Footer-Filter
    const sps = Array.isArray(spTree['p:sp']) ? spTree['p:sp'] : (spTree['p:sp'] ? [spTree['p:sp']] : []);
    for (const sp of sps) {
        if (isFooterShape(sp)) continue;
        collectText(sp, out);
    }

    // Bilder (p:pic) — Text in alt/description moeglich
    if (spTree['p:pic']) collectText(spTree['p:pic'], out);

    // Tabellen, Diagramme, Embeds (p:graphicFrame)
    if (spTree['p:graphicFrame']) collectText(spTree['p:graphicFrame'], out);

    // Gruppen (p:grpSp) — rekursiv weiter (selten, aber moeglich)
    if (spTree['p:grpSp']) collectText(spTree['p:grpSp'], out);

    // Verbindungs-Linien (p:cxnSp) — meist ohne Text, zur Sicherheit mit
    if (spTree['p:cxnSp']) collectText(spTree['p:cxnSp'], out);
...
function extractStructure(slideObj) {
    ...
    try {
        ...
        const sps = Array.isArray(spTree['p:sp']) ? spTree['p:sp'] : (spTree['p:sp'] ? [spTree['p:sp']] : []);
        for (const sp of sps) {
            if (isFooterShape(sp)) continue; // Footer/Datum/Foliennummer ignorieren
...
async function analyzePptx(filePath) {
    ...
    for (let i = 0; i < slideFiles.length; i++) {
        ...
        // Text extrahieren — OHNE Fusszeile/Datum/Foliennummer
        const textParts = collectSlideText(slideObj);
